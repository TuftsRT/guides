...
```bash
[tutln01@login-p01 ~]$ srun -p preempt --qos=preempt -t 1-2:30:00 -n 2 --mem=2g --x11=first --pty bash
...
```bash
[tutln01@login-p01 ~]$ srun -p preempt --qos=preempt  -t 1-2:30:00 -n 2 --mem=4g --gres=gpu:a100:1 --constraint="a100-40G" --pty bash
```
