...
1. From the login node, load R module (e.g. `r/4.4.3`)
...
5. Exit from R command line interface:

   `> q()`

1. It is important to `$ exit` the interactive session when finished to free up resources for other users.
...
1. Upload your R script or create your own on HPC cluster

1. Go to the directory/folder which contains your R script, e.g.:
