...
The tunneling workflow requires a GitHub or Microsoft account. If you do not already have a GitHub account, you can create one at [GitHub](https://github.com/).

## OnDemand VSCode Server

1. Log in to [Tufts HPC Open OnDemand](https://ondemand-prod.pax.tufts.edu/).

1. Select **VSCode Server** from the _Interactive Apps_ menu.

   ![vscode-launch](../assets/el9/newondemand-vscode-launch.png)

1. Fill out the form and click **Launch**. The VSCode Server session will run on a compute node using the resources you requested.
...
1. When finished, **delete** the VSCode Server session to free resources for other users.

```{tip}
If you are using VSCode via the VSCode Server, the _File_, _Edit_, _Go_, and _Run_ menus are not on the top of the screen like you may be used to on your local computer. These are stored in a menu with three lines (the hamburger menu) on the far left of the VSCode window.
```
...
To use extensions, they need to be installed and the system must meet all prerequisites indicated on the official extension page. These are optional, but they add additional functionality which may be important for data science workflows.
...
You can follow the instructions [here](https://www.tecmint.com/ssh-passwordless-login-using-ssh-keygen-in-5-easy-steps/) to set up your SSH keyless access. This will make your life much easier in the long run.
...
1. **Allocate Resources on HPC Cluster**
...
1. **Load Cluster VSCode CLI Module**
...
1. **Then Configure and Start Tunnel**
...
1. **Authentication**
   ...
   ![vscodecli-browser](../assets/el9/vscode-tunnel-browser.png)

   Once you complete these steps, you will see a message that says:

   > _"Congratulations, you're all set! Your device is now connected."_
