...
The cluster utilizes Slurm QOS to manage special cases and exceptions to the defaul resource and time limits.  
...
Each partition has a time limit that a jobs runtime cannot exceed.  Most use cases should be able to complete within these time ranges, but we understand thier are exception.  Please contact us if you need access to longer runtimes.
