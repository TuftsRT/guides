...
1. Web browser access via the [OnDemand Portal](https://ondemand-prod.pax.tufts.edu)
...
```{important}
   SSH login host is login-prod.pax.tufts.edu
```
...
- Explanation of the Command:
  ...
  - `your_utln@login-prod.pax.tufts.edu`: Replace `your_utln` with your actual Tufts username. This is the address of the remote HPC cluster.
  ...
  - You can add the argument `-X` to your ssh command to enable the X11 graphical forwarding in your SSH connection. X Window System must be installed on your computer. That looks like this:
    `ssh -X your_utln@login-prod.pax.tufts.edu`
...
Now you are on a **Login Node** of the cluster (login-p0x) and in your **Home Directory** (~ or _/cluster/home/your_utln_).

`$ [your_utln@login-p01 ~]`

There are 3 login nodes: `login-p01`, `login-p02`, `login-p03`
