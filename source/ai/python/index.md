---
tags: python vs-code google-colab conda jupyter hpc
---

# Python for Research and Data Science

Welcome to the Tufts RT Python Setup documentation!

If you are looking for general information on what Python is and whether it's right for you, start with Python for Research and Data Science.

Note that this guide best for users interesetd in statistics, machine learning, data analysis, data science, artificial intelligence, and data visualization. Application developers and other users may be better served by other online documentation.

```{gallery-grid}
---
grid-columns: 1
---
- header: "{fas}`book` Python for Research and Data Science"
  content: "Start here to begin learning how to set up Python."
  link: "00-intro-to-python.html"

- header: "{fas}`book` Which Python Setup is Right for You?"
  content: "Our recommendations for Python setups based on your needs and experience level, with descriptions to help you decide what's right for you."
  link: "10-which-python-setup.html"

- header: "{fas}`book` Google Colab"
  content: "Learn how to access Google Colab at Tufts, our recommendation for beginner-to-intermediate Python programmers and Python learners."
  link: "20-google-colab.html"

- header: "{fas}`book`  Microsoft Visual Studio Code with Conda (Miniforge): Installation and Setup"
  content: "Learn how to set up Microsoft Visual Studio Code with Conda (Miniforge), our recommended local Python configuration for intermediate-to-advanced users."
  link: "30-vs-code-with-miniforge.html"

- header: "{fas}`book` Microsoft Visual Studio Code with Conda (Miniforge): Getting the Most Out of Your Setup"
  content: "When you've finished installing and configuring Microsoft Visual Studio Code with Conda, click here to learn how to use your setup effectively."
  link: "40-VS-code-and-conda-users-guide.html"

- header: "{fas}`book` Jupyter on the Tufts HPC Cluster"
  content: "Learn how to launch Jupyter Lab on the HPC cluster via OnDemand, use conda environments as kernels, and transfer files."
  link: "50-jupyter-on-hpc.html"

- header: "{fas}`book` Python Package Management with Conda and Pip"
  content: "Learn to create, export, and share conda environments and pip requirements files for reproducible Python projects."
  link: "60-package-management.html"

- header: "{fas}`book` Python Data Science: Tutorials and Key Libraries"
  content: "Hands-on introductions to pandas, matplotlib, seaborn, and scikit-learn for data analysis, visualization, and machine learning."
  link: "70-data-science-tutorials.html"

- header: "{fas}`book` AI and Machine Learning Workflows on the HPC"
  content: "Guides for setting up and running PyTorch, TensorFlow, and Hugging Face Transformers on the Tufts HPC cluster with SLURM."
  link: "80-ai-ml-workflows.html"


```

> **To Get Support:** - Please email datalab-support@elist.tufts.edu for Python support at Tufts
